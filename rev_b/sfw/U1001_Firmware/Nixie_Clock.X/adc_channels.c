...
void adcChannelsInitialize(void) {
    ...
    ADCIMCON2bits.DIFF20 = 0; // Single ended mode
    ADCIMCON2bits.SIGN20 = 0; // unsigned data format
    if (RTC_HARDSTRAP_PIN == 0) {
        ADCIMCON2bits.DIFF21 = 0; // Single ended mode
        ADCIMCON2bits.SIGN21 = 0; // unsigned data format
    }
    ...
    ADCGIRQEN1bits.AGIEN19 = 1;     // enable data ready 19 IRQ
    ADCGIRQEN1bits.AGIEN20 = 1;     // enable data ready 20 IRQ
    if (RTC_HARDSTRAP_PIN == 0) ADCGIRQEN1bits.AGIEN21 = 1;     // enable data ready 21 IRQ
    ADCGIRQEN2bits.AGIEN43 = 1;     // enable data ready 43 IRQ
    ADCGIRQEN2bits.AGIEN44 = 1;     // enable data ready 44 IRQ
    
    // set IRQ priorities
    setInterruptPriority(ADC_Data_19, 1);
    setInterruptPriority(ADC_Data_20, 1);
    if (RTC_HARDSTRAP_PIN == 0) setInterruptPriority(ADC_Data_21, 1);
    setInterruptPriority(ADC_Data_43, 1);
    setInterruptPriority(ADC_Data_44, 1);
    
    // set IRQ subpriorities
    setInterruptSubpriority(ADC_Data_19, 1);
    setInterruptSubpriority(ADC_Data_20, 1);
    if (RTC_HARDSTRAP_PIN == 0) setInterruptSubpriority(ADC_Data_21, 1);
    setInterruptSubpriority(ADC_Data_43, 2);
    setInterruptSubpriority(ADC_Data_44, 3);
    
    /* Configure common scan */
    ADCCSS1bits.CSS19 = 1;          // Enable Channel 19 for common scan
    ADCCSS1bits.CSS20 = 1;          // Enable Channel 20 for common scan
    if (RTC_HARDSTRAP_PIN == 0) ADCCSS1bits.CSS21 = 1;          // Enable Channel 21 for common scan
    ADCCSS2bits.CSS43 = 1;          // Enable Channel 43 for common scan
    ADCCSS2bits.CSS44 = 1;          // Enable Channel 44 for common scan
    
    // clear interrupt flags
    clearInterruptFlag(ADC_Data_19);
    clearInterruptFlag(ADC_Data_20);
    if (RTC_HARDSTRAP_PIN == 0) clearInterruptFlag(ADC_Data_21);
    clearInterruptFlag(ADC_Data_43);
    clearInterruptFlag(ADC_Data_44);
    
    // enable data ready interrupts
    enableInterrupt(ADC_Data_19);
    enableInterrupt(ADC_Data_20);
    if (RTC_HARDSTRAP_PIN == 0) enableInterrupt(ADC_Data_21);
    enableInterrupt(ADC_Data_43);
    enableInterrupt(ADC_Data_44);
    
}

void __ISR(_ADC_DATA19_VECTOR, IPL1SRS) ADCData19ISR(void) {
...
void __ISR(_ADC_DATA20_VECTOR, IPL1SRS) ADCData20ISR(void) {
...
void __ISR(_ADC_DATA21_VECTOR, IPL1SRS) ADCData21ISR(void) {
    ...
    // clear IRQ
    clearInterruptFlag(ADC_Data_21);

...
void platformETCInitialize(void) {
    ...
    DS1683ETCInitialize(PLATFORM_ETC_ADDR, &error_handler.flags.platform_etc);
...
void miscI2CDevicesPrintStatus(void) {
    ...
    if (ETC_HARDSTRAP_PIN == LOW) DS1683PrintStatus(PLATFORM_ETC_ADDR, &error_handler.flags.platform_etc);
    
    if (RTC_HARDSTRAP_PIN == LOW) DS3231PrintStatus(BACKUP_RTC_ADDR, &error_handler.flags.backup_rtc);
